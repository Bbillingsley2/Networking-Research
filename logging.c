...
typedef enum log{
    ...
    NETWORK_DEBUG=0x2,
    SERVER_DEBUG=0x4,
    ALL=NETWORK_DEBUG|BASIC_DEBUG|SERVER_DEBUG
}flags;
...
{
    if (LEVEL & LEVEL_TRACED) {
    (void) fprintf(stderr, "%s [LOG LEVEL: %d] [%s() %s:%d] ", get_current_time(), LEVEL_TRACED, __func__, __FILE__, __LINE__);
//    (void) fprintf(stderr, "%s [LOG LEVEL: %s] [%s() %s:%d] ", get_current_time(), LEVEL[LEVEL_TRACED], __func__, __FILE__, __LINE__);
